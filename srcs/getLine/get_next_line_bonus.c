...
/*   Updated: 2019/12/31 16:33:04 by majermou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int						get_next_line(int fd, char **line)
{
	...
	var_bol = 0;
	ret = 1;
	if (!error_check(fd, line, &tab[fd]))
		return (-1);
	while (1)
	{
		tmp = *line;
		if (!(*line = ft_strjoin(*line, tab[fd])))
			return (-1);
		free(tmp);
		if (!ret || check_break_line(tab[fd]) >= 1)
			break ;
		if ((ret = read(fd, tab[fd], BUFFER_SIZE)) == -1)
			return (-1);
		tab[fd][ret] = '\0';
		var_bol = 1;
	}
	return (dealocate(line, &tab[fd], ret, var_bol));
}
